...
class Doxypy(object):
	def __init__(self):
		...
		self.defclass_re = re.compile("^(\s*)((def|class)\s+((\w+)\s*\(?.*)):")
		self.empty_re = re.compile("^\s*$")
		self.hashline_re = re.compile("^\s*#.*$")
		self.importline_re = re.compile("^\s*(import |from .+ import)")

		self.multiline_defclass_start_re = re.compile("^(\s*)((def|class)(\s*)([\w]+)\s*\(?.*)$")
	...
	def __closeComment(self):
		...
		if self.comment:
			block = []
			if self.curDef == "class":
				block = self.makeClassBlock(self.curName)
			elif self.curDef == "def":
				block = self.makeFunctionBlock(self.curName)
			else:
	...
	def startCommentSearch(self, match):
		...
		self.curDef = match.group(3)
		self.curName = match.group(5)
	...
	def appendCommentLine(self, match):
		...
		# multiline start
			...
			self.originalIndent = line.find(activeCommentDelim)
		# multiline end
		...
		# in multiline comment
			...
			self.comment.append(self.fsm.current_input[:self.originalIndent].strip() + self.fsm.current_input[self.originalIndent:])
	...
	def makeCommentBlock(self):
		...
		if len(commentLines) > 0:
		    doxyStart += commentLines[0]
		
		commentLines = map(lambda x: "%s# %s" % (self.indent, x), commentLines[1:])
		l = [self.indent + doxyStart]
		l.extend(commentLines)
			 
		return l
	def makeFunctionBlock(self, functionName):
		"""Indents the current comment block with respect to the current
		indentation level.

		@returns a list of indented comment lines
		"""
		#functionDef = "".join(self.defclass)
		#match = self.defclass_re.match(functionDef)
		#pos = functionDef.find(match.group(5))
		#functionDef = functionDef[pos:pos+len(match.group(5))]
		
		doxyStart = " ## @property " + functionName
		commentLines = self.comment
		
		commentLines = map(lambda x: "%s    # %s" % (self.indent, x), commentLines)
		self.defclass[-1] += doxyStart  #Preserve line numbers by outputing to the end of the class def, instead of in front
		self.defclass.extend(commentLines)
		
		return []
	def makeClassBlock(self, className):
		"""Indents the current comment block with respect to the current
		indentation level.

		@returns a list of indented comment lines
		"""
		doxyStart = "## @class " + className
		commentLines = self.comment
		
		commentLines = map(lambda x: "# %s" % (x,), commentLines)
		l = [doxyStart]
		l.extend(commentLines)
		l.append("")
		self.finalOutput.extend(l)
		
		self.defclass.extend([""]*len(commentLines)) #Preserve line numbers
		return []
	...
	def parse(self, input):
		...
		self.output.extend(self.finalOutput)
		self.finalOutput = []
